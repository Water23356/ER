...
namespace Dev2
{
    public class NearbyAgentScanner : MonoBehaviour
    {
        public float scanRadius = 5f; // 扫描半径
        public LayerMask detectionLayers; // 参与检测的层

        // 获取周围物体的方法
        public STG_OA_Agent[] ScanNearbyObjects()
        {
            ...
            List<STG_OA_Agent> results = new List<STG_OA_Agent>();
            foreach (Collider2D collider in colliders)
            {
                if (collider.gameObject == gameObject) continue;
                var agent = collider.GetComponent<STG_OA_Agent>();
                if (agent != null)
                    results.Add(agent);
            }
