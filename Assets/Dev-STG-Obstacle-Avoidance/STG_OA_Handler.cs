...
namespace Dev2
{
    [RequireComponent(typeof(Rigidbody2D), typeof(NearbyAgentScanner), typeof(CircleCollider2D))]
    public class STG_OA_Handler : MonoBehaviour
    {
        ...
        private NearbyAgentScanner m_scanner;

        [SerializeField]
        private Vector2 preferredVelocity;

        public NearbyAgentScanner scanner
        {
            get
            {
                if (m_scanner == null)
                    m_scanner = GetComponent<NearbyAgentScanner>();
                return m_scanner;
            }
        }

        [SerializeField]
        private float width_ORCA = 0.01f;

        [SerializeField]
        private float defualtRadius = 0.1f;

        private float radius
        {
            get
            {
                return GetComponent<CircleCollider2D>()?.radius ?? defualtRadius;
            }
        }
        ...
        private void AutoDoge()
        {
            var agents = scanner.ScanNearbyObjects();
            List<AreaLine> lines = new List<AreaLine>();
            for (int i = 0; i < agents.Length; i++)
            {
                if (HandleObjectORCA(agents[i], out AreaLine line))
                {
                    lines.Add(line);
                }
            }
            ...
            if (lines.Count == 0)//没有边界, 则意味着无需调整
            {
                ...
                rigidbody.velocity = PreferredVelocity;
                return;
            }
            ...
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                //求垂足有效点
                if (AreaLine.GetFootOfPerpendicular(lines[i], Vector2.zero, out var orcap))
                {
        ...
        private bool HandleObjectORCA(STG_OA_Agent agent, out AreaLine orca)
        {
            var speedB = agent.velocity;

            var AB = (Vector2)agent.transform.position - (Vector2)transform.position;
            var r = agent.radius + radius;
            var spAB = -speedB;//A对B的相对速度

            // 处理速度非常接近的情况
            if (spAB.sqrMagnitude < 0.001f)
            {
                orca = new AreaLine();
                return false;
            }
            ...
            float rad_AB_v = Mathf.Asin(r / AB.magnitude);//AB 到 切线的角度大小
            ...
            //绘制障碍边界线
            DrawCircle(agent.transform.position, r, color_barrier);
            ...
            if (inTriangle)
            {
                if (inCircle)//在扇形范围内
                {
                    ...
                    Debug.DrawLine(transform.position, transform.position + (Vector3)change, Color.red);
#endif
                    //change 速度改变方向
                    orca = new AreaLine()
                    {
        ...
        private void DrawCircle(Vector2 pos, float r, Color color)
        {
            ...
            for (int i = 0; i <= count; i++)
            {
                ...
                Debug.DrawLine(pos + point, pos + next, color);
                point = next;
            }
        }
